  ...
  return (
    <section className="comp-data-panel">
      ...
      {rows.map(row => {
        ...
        >
          <span className="comp-standings-pos">{row.pos}</span><ApiPlayerImage playerId={resolvedId} name={row.name} preferredSrc={row.img} fallbackSrc="/assets/players/neutral-player.svg" alt="" loading="lazy"/><span title={row.name}>{row.name}</span><small title={row.team}>{row.team}</small>{row.cal!=null&&<span style={{fontFamily:"'Barlow Condensed', sans-serif",fontSize:11,fontWeight:800,color:row.cal>=80?'#97cc0d':row.cal>=72?'#f59e0b':'#888',marginRight:8,padding:'1px 6px',border:'1px solid #2a2a2a',borderRadius:3}} title="Calibre rating">CAL {row.cal}</span>}<strong>{row[stat]}</strong>
        </div>
        );
      })}
  ...
  return (
    <button type="button" onClick={onOpen} style={{ display:'flex', flexDirection:'column', gap:8, textAlign:'left', background:'#0c0c0e', border:'1px solid #1c1c1c', borderRadius:10, padding:'12px 14px', cursor:'pointer', color:'#ddd', width:'100%' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', fontSize:11, color:'#888' }}>
        ...
        <span style={{ display:'inline-flex', alignItems:'center', gap:4, color:'#97cc0d' }}><MessageSquare size={11}/> Open thread</span>
      </div>
    </button>
  );
}
...
.mr-featured-head span { display:inline-flex; align-items:center; gap:6px; font:800 10px/1 'IBM Plex Mono',monospace; letter-spacing:.12em; text-transform:uppercase; color:#97cc0d; }
.mr-featured-head span i { width:6px; height:6px; border-radius:50%; background:#97cc0d; display:inline-block; }
...
.mr-fixture-row:hover .mr-fixture-teams { color:#97cc0d; }
.mr-fixture-row small { color:#8d929b; font:600 10px/1 'Barlow',sans-serif; flex:none; }
.mr-fulltable { display:inline-flex; align-items:center; gap:5px; margin-top:8px; padding:6px 0; background:none; border:0; color:#97cc0d; font:800 9px/1 'IBM Plex Mono',monospace; letter-spacing:.08em; text-transform:uppercase; cursor:pointer; }
.mr-analysis-box { border:1px solid rgba(151,204,13,.18); background:rgba(151,204,13,.04); border-radius:10px; padding:14px; margin:14px 0 4px; }
.mr-analysis-box .matchroom-label { display:inline-flex; align-items:center; gap:6px; font:800 9px/1 'IBM Plex Mono',monospace; letter-spacing:.1em; text-transform:uppercase; color:#97cc0d; }
...
.mr-analysis-key svg { color:#97cc0d; flex:none; margin-top:1px; }
...
.comp-fulltable-row .comp-standings-pts { color:#97cc0d; font-weight:800; }
...
.comp-field-round { margin:2px 0 8px; color:#97cc0d; font:700 9px/1 'IBM Plex Mono', monospace; letter-spacing:.1em; text-transform:uppercase; }
