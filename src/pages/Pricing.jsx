...
function PlanCard({ plan }) {
  function handleCta() {
    if (plan.disabled) { window.alert('Waitlist coming soon. Drop us your email at team@calibrefootball.com'); return; }
    if (plan.paymentEnabled && plan.lemonUrl) window.open(plan.lemonUrl, '_blank', 'noopener');
  }
  return <div className={`pricing-card ${plan.badge === 'MOST POPULAR' ? 'pricing-card--featured' : ''} ${plan.disabled ? 'pricing-card--disabled' : ''}`}>
    {plan.badge && <div className={`pricing-badge ${plan.badge === 'MOST POPULAR' ? 'pricing-badge--hot' : 'pricing-badge--soon'}`}>{plan.badge === 'COMING SOON' && <Clock size={11} style={{marginRight:4}}/>}{plan.badge}</div>}
    <div className="pricing-name">{plan.name}</div>
    <div className="pricing-price"><span className="pricing-amount">{plan.price}</span><span className="pricing-period">{plan.period}</span></div>
    <p className="pricing-tagline">{plan.tagline}</p>
    <ul className="pricing-features">{plan.features.map(feature=><li key={feature}><Check size={14}/>{feature}</li>)}</ul>
    <button type="button" className={`pricing-cta pricing-cta--${plan.ctaStyle} ${plan.disabled ? 'pricing-cta--waitlist' : ''}`} onClick={handleCta}>{plan.id === 'free' ? 'Get started free' : plan.cta}{!plan.disabled && <ArrowRight size={14}/>}</button>
  </div>;
}
...
export default function Pricing() {
  return <div className="page pricing-page">
    ...
    <div className="pricing-grid">{PLANS.map(plan=><PlanCard key={plan.id} plan={plan}/>)}</div>

    <section className="market-insights-mini" aria-label="Optional betting-market intelligence">
      <div className="market-insights-mini__icon"><BarChart3 size={17}/></div>
      <div className="market-insights-mini__copy"><strong>Optional betting-market intelligence</strong><span>Small add-on lane for match probability, xG movement and goals-market context. Kept separate from the core football product.</span></div>
      <div className="market-insights-mini__tags"><span>Match probability</span><span>Goals outlook</span><span>BTTS context</span><span>Partner odds</span></div>
      <div className="market-insights-mini__status"><ShieldCheck size={14}/><span>Coming with official partner · 18+</span></div>
    </section>

    <p className="pricing-footer">All plans include access to current-season data. Paid plans powered by Lemon Squeezy — cancel anytime. Questions? <a href="mailto:team@calibrefootball.com">team@calibrefootball.com</a></p>
  </div>;
}
