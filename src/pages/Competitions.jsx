...
export default function Competitions() {
  ...
  useEffect(()=>{ const fallback=snapshotFor(active); const fallbackMatch=featuredMatchFor(active); const tournamentOnly=active.standingsMode==='tournament-only'; setStandings(tournamentOnly?[]:fallback.standings); setScorers(fallback.scorers); setCreators(fallback.creators); setFeaturedMatch(fallbackMatch); setFixtures([fallbackMatch]); setForumMatch(null); setStandingsUnavailable(tournamentOnly?'Competition-only standings will appear when the UWCL feed publishes a table. Domestic-league rows are deliberately not substituted.':''); setLiveState({standings:false,scorers:false,creators:false,fixture:false}); let cancelled=false; setLoading(true);
